...
export default function AdminHomePage() {
  const { data, isLoading } = useSWR<HomeContent>("/api/content/homepage", fetcher)
  ...
  const handleRemoveImage = (projectId: number) => {
    ...
    setContent({
      ...content,
      projects: content.projects.map((p) =>
        p.id === projectId ? { ...p, image: undefined } : p
      ),
    })
  }
  ...
  const handleAddProject = () => {
    ...
    setContent({
      ...content,
      projects: [...content.projects, { id: newId, title: "Nuevo proyecto" }],
    })
  }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-[#1a1a1a]">Proyectos Destacados</h1>
          </div>
        ...
        <p className="text-muted-foreground mb-8">
          Estos son los proyectos que aparecen en la sección "Proyectos seleccionados" de la página de inicio.
        </p>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {content.projects.map((project) => (
            <div key={project.id} className="bg-white rounded-xl p-4">
              <div className="aspect-[4/3] bg-gray-200 rounded-lg overflow-hidden relative mb-3">
                {project.image ? (
                  <>
                    <Image
