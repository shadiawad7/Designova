...
export async function GET() {
  try {
    ...
    const result = await pool.query<FotoRow>("SELECT id, foto FROM foto_der")
    return NextResponse.json({ items: result.rows })
  } catch (error) {
    console.error("Error fetching foto_der:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await pool.query<FotoRow>(
      "INSERT INTO foto_der (id, foto) VALUES ($1, $2) RETURNING id, foto",
      [id, foto ?? null]
    )
    return NextResponse.json({ item: result.rows[0] })
  } catch (error) {
    console.error("Error creating foto_der:", error)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const pool = getDbPool()
    const result = await pool.query<FotoRow>(
      "UPDATE foto_der SET foto = $1 WHERE id = $2 RETURNING id, foto",
      [foto ?? null, id]
    )
    return NextResponse.json({ item: result.rows[0] })
  } catch (error) {
    console.error("Error updating foto_der:", error)
